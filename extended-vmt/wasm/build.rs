...
fn main() {
    ...
    let idl_path = cargo_toml_path.clone().join("extended_vmt.idl");
    let client_path = outdir_path.clone().join("extended_vmt_client.rs");
    ...
    fs::copy(client_path, cargo_toml_path.join("extended_vmt_client.rs"))
        .unwrap();
}
