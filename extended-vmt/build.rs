...
fn main() {
    ...
    let outdir_idl_path = outdir_path.clone().join("extended_vmt.idl");
    let outdir_client_path = outdir_path.clone().join("extended_vmt_client.rs");
    ...
    sails_idl_gen::generate_idl_to_file::<extended_vmt_app::ExtendedVmtProgram>(outdir_idl_path.clone())
        .unwrap();
    ...
    fs::copy(outdir_client_path, idl_and_client_dir_path.clone().join("extended_vmt_client.rs"))
        .unwrap();

    fs::copy(outdir_idl_path, idl_and_client_dir_path.join("extended_vmt.idl"))
        .unwrap();
}
