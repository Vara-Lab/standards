...
impl ExtendedService {
    ...
    pub fn mint(&mut self, to: ActorId, token_metadata: TokenMetadata) {
        if !self.get().minters.contains(&Syscall::message_source()) {
    ...
    pub fn burn(&mut self, from: ActorId, token_id: TokenId) {
        if !self.get().burners.contains(&Syscall::message_source()) {
...
impl ExtendedService {
    fn ensure_is_admin(&self) {
        if !self.get().admins.contains(&Syscall::message_source()) {
            panic!("Not admin")
        };
    }
}
